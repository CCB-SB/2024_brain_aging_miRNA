...
#snakemake = readRDS("snakemake_volcano_scatter_plot_microglia.rds")

set.seed(snakemake@params$parameters_props$set_seed)

print("volcano_scatter_plot_microglia")
...
# save rdata
#saveRDS(snakemake, file = "snakemake_volcano_scatter_plot_microglia.rds")
...
#foreach (i=1:length(comps)) %dopar% {
  ...
  plot_df[ttest_adj_pval >= sig_lvl, ttest_adj_cat_mod:=ns_string]
  plot_df[ttest_adj_pval < sig_lvl, ttest_adj_cat_mod:=s_string]
  plot_df[logfc >= log_fc_thres, ttest_adj_cat_mod:=up_string]
  plot_df[logfc <= -log_fc_thres, ttest_adj_cat_mod:=down_string]
  ...
  colors_ttest_adj_mod = colors[names(colors) %in% plot_df$ttest_adj_cat_mod]
  ...
  sub_plot_df$ttest_adj_cat_show_mod = gsub("mmu-miR-", "", sub_plot_df$feature)
  ...
  mask_ttest_adj_mod = ((sub_plot_df$ttest_adj_cat_mod != up_string) & (sub_plot_df$ttest_adj_cat_mod != down_string)) 
  ...
  sub_plot_df[mask_ttest_adj_mod,]$ttest_adj_cat_show_mod = ""
  ...
  p2_mod = ggplot(sub_plot_df, aes(x=logfc, y=ttest_adj_pval_log10, color=ttest_adj_cat_mod)) + 
    geom_point(size = 0.75) +
    geom_hline(yintercept = -log10(sig_lvl), color="lightgrey", linetype="dashed", size = 0.25) +
    geom_vline(xintercept = -log_fc_thres, color="lightgrey", linetype="dashed", size = 0.25) + 
    geom_vline(xintercept = log_fc_thres, color="lightgrey", linetype="dashed", size = 0.25) +
    geom_text_repel(aes(label = ttest_adj_cat_show_mod),
                    max.time = repel_max_time, max.iter = 1e6,
                    max.overlaps = Inf,
                    box.padding = 0.1, 
                    point.patting = 0.1, 
                    ...
                    size = label_font_size/11.04*3.88, family = plots_props$font_family, color = "black") +
    xlab(x_axis_name_p2) + ylab(y_axis_name_p2) +
    scale_color_manual(name="", values=colors_ttest_adj_mod) + 
    theme(legend.position = "none", 
          plot.background = element_rect(fill="transparent"),
          panel.background = element_rect(fill="transparent"),
          text = element_text(family = plots_props$font_family, size = plots_props$font_size), 
          ...
          legend.text = element_text(family = plots_props$font_family, size = plots_props$font_size),
          legend.title = element_text(family = plots_props$font_family, size = plots_props$font_size, face = "bold")) + 
    guides(col=guide_legend(nrow=length(colors_ttest_adj_mod)))
  ...
  if(is.null(group)) {
    ...
    # 6x6
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw.labels_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p1, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw.labels_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p1, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.labels_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p2, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.labels_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p2, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.labels_quad_mod.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p2_mod, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.labels_quad_mod.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p2_mod, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw.labels_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p3, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw.labels_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p3, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj.labels_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p4, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj.labels.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p4, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate.labels_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p5, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate.labels_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p5, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p6, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p6, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p7, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p7, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p8, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p8, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p9, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p9, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate_quad.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p10, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate_quad.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p10, width = plots_props$image_width*2/3, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    
    # 6x9
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw.labels.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p1, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw.labels.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p1, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.labels.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p2, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.labels.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p2, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw.labels.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p3, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw.labels.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p3, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj.labels.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p4, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj.labels.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p4, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate.labels.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p5, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate.labels.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p5, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi, bg = "transparent")
    
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p6, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.raw.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p6, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p7, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.ttest.adj.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p7, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p8, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.raw.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p8, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p9, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.wilcox.adj.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p9, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate.png", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p10, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
    #ggsave(file = sprintf("%s/%s__%s_vs_%s%s%s.cohend_estimate.svg", output_folder, prop, c$g1, c$g2, paired_string, sub_string), plot = p10, width = plots_props$image_width, height = plots_props$image_height, unit = plots_props$image_units, dpi = plots_props$dpi)
  } else {
