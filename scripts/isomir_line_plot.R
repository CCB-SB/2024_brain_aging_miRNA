...
order_of_x_axis = snakemake@params$order_of_x_axis
...
for (i in 1:length(manual_plot_list)) {
  ...
  for (group_1 in unique(annot[[plot_group]])) {
    ...
    #plot_df$comp_names = unname(unlist(xticks_names[[comp_group]][plot_df$comp]))
    plot_df$comp_factor = factor(plot_df$comp, levels = (order_of_x_axis))
    ...
    #x_max = max(unique(annot[[comp_group]]))
    
    y_maxi[[group_1]] = max(expr_feature_plot_group_medians[[group_1]]$median)
    
    # create bar plot
    line_plots[[group_1]] = ggplot(plot_df, aes(x = comp_factor, y = median, group=iso_type, color=colour)) + 
                            geom_line() +
                            ggtitle(plot_title) +
                            xlab("") +
                            ylab("") +
                            scale_x_discrete(limits = order_of_x_axis, expand = c(0, 0), labels = unlist(xticks_names[[comp_group]])) +
