...
#snakemake = readRDS("snakemake_hclust_expression_zscores_heatmap_plot.rds")

data.table::setDTthreads(1)
set.seed(snakemake@params$parameters_props$set_seed)

print("hclust_expression_zscores_heatmap_plot")
...
plot_heatmap = function(df, row_names, i, identifier_column, prop, scientific_numbers, legend_title, colors, xticks_names, plot_props, width, height, filename_fig){
  ...
  if (length(unique(as.numeric(df))) <= 2) {
    ...
    mini = min(df)
    #col_fun = colorRamp2(c(0,maxi), hcl_palette = "Greens", reverse = TRUE)
    col_fun = colorRamp2(c(mini,maxi), c("#FFF6D6","#D5A021"), reverse = TRUE)
    col_fun_legend = col_fun
  }
  ...
  print(sprintf("Old names: %s", paste(colnames(df), collapse = ", ")))
  colnames(df) = unlist(xticks_names[[sprintf("%s_sample", prop)]][colnames(df)])
  print(sprintf("New names: %s", paste(colnames(df), collapse = ", ")))
...
# save rdata
#saveRDS(snakemake, file = "snakemake_hclust_expression_zscores_heatmap_plot.rds")
...
output_folder = sprintf("%s/%s_%s/results_%s/figures/cv_clustering/heatmap_complex", results_folder, data_input$rna_class, data_input$detection_rate, data_input$data_sub_set)
...
for(i in top_list) {
  ...
  for(prop in properties) {
    ...
    top_cov = fread(sprintf("%s/%s_%s/results_%s/matrices/cv/%s.csv", results_folder, data_input$rna_class, data_input$detection_rate, data_input$data_sub_set, i))
    
    # expr
    scientific_numbers = FALSE
    
    expr_filtered = expr[expr[[data_input$rna_class]] %in% top_cov$miRNA,]
    ...
    scale_key = "row"
    if(scale_key != "none"){
      scale_row = scale_key %in% c("row", "both")
      scale_col = scale_key %in% c("column", "both")
      expr_filtered_zscores = scale(expr_filtered, scale_row, scale_col) #base::scale(t(median_table_adj[i,]))
      expr_filtered_zscores[is.na(expr_filtered_zscores)] = 0
      expr_filtered_zscores = abs(expr_filtered_zscores)
    }   
    ...
    legend_title = sprintf("Abs. stand. expr. (%s)", gsub("_norm", "", data_input$norm))
    
    filename = sprintf("%s/top_%s", output_folder, i)
  
    plot_heatmap(expr_filtered_zscores, row_names, i, identifier_column, prop, scientific_numbers, legend_title, colors, xticks_names, plot_props, plot_props$image_width, plot_props$image_height, filename)
