...
for (detection_rate in detection_rate_list) {
  ...
  # venn plot
  p = ggplot(number_of_mirnas_df, aes(x=reorder(tissue, -number_of_mirnas), y=number_of_mirnas, fill=tissue)) + 
  ...
  for (th in intersection_th_list) {
    ...
    png(sprintf("%s/number_of_expressed_miRNAs_per_%s_th=%s_counts_detection_rate=%s_intersection_th=%s.png", output_folder_upset, tissue, min_detect_raw, detection_rate, th), width = plots_props$image_width, height = plots_props$image_height, units = plots_props$image_units, res = plots_props$dpi)
    ...
    dev.off()
    svglite(sprintf("%s/number_of_expressed_miRNAs_per_%s_th=%s_counts_detection_rate=%s_intersection_th=%s.svg", output_folder_upset, tissue, min_detect_raw, detection_rate, th), width = plots_props$image_width / 2.54, height = plots_props$image_height / 2.54)
    upset_plot = draw(upset_plot, padding = unit(c(-0.1, -0.25, 0.25, 0.25), "cm"));  # bottom, left, top and right margins
    decorate_annotation("miRNA\noverlap", {grid.text(cs[co], x = 1:nc, y = unit(cs[co], "native") + unit(1, "mm"), gp = gpar(fontsize = 6), rot = 90, vjust = 0.35, hjust = 0.15, just = "bottom", default.units = "native")})
    dev.off()
  }
